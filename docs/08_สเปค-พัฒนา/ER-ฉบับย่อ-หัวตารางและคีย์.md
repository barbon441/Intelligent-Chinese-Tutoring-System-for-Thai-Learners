...
>
> **ใช้หน้าไหนตอนไหน:** อธิบายหน้าห้อง/ซ้อมตอบ → ใช้หน้าก้าวที่ 1-6 ทีละหน้า (หน้าละไม่กี่กล่อง เส้นแทบไม่ตัดกัน) · ใส่เล่มหรือโปสเตอร์ → ใช้หน้า "ภาพรวมทั้งระบบ" (จัดให้เส้นตัดกันน้อยสุด — โชว์ 10 วินาทีให้เห็นว่า "ครบ 23 ตาราง" แล้วสลับไปหน้าก้าว อย่าไล่เส้นบนหน้านี้) · *(3 ก.ย. เคยลองเพิ่มหน้าภาพรวมแบบเรียงตามเรื่อง — บอลเคาะถอดออก: 36 เส้นหน้าเดียวยังไงก็พันกัน หน้าก้าว ①-⑥ คือคำตอบอยู่แล้ว)*
...
> **ตอบ:** ใช้ `sessions.form_id` ชี้ `exam_forms` ตัวเดียวกันทั้งสองรอบ ตามกฎ PL-09 ครับ *(ทีมรับตามแนวทางอาจารย์แล้ว 3 ก.ย. — ชุดเดียว = ข้อสอบเสมือน HSK1 ทุกคนทำ · เหลือถามอาจารย์แค่จำนวนข้อ 40 เต็มหรือฉบับย่อ)*
...
| `words` | คำศัพท์จีน 1 คำ | `id` | `categories` *(ประกาศในพิมพ์เขียว 3 ก.ย. — DB จริงวันนี้ยังเป็นเลขลอย จะบังคับจริงตอน m7-2)* |
...
| `words.category` | ผังมีเส้นไป `categories` แล้ว (พิมพ์เขียว) แต่ DB จริงวันนี้ยังเป็นเลขลอย ไม่มี constraint — จะผูกจริงตอน m7-2 |
